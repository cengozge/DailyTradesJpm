    ...
    public static List<TradeInstruction> generateData(){
        ...
        for (int i = 1; i <= numberOfRecords; i++){
            ...
            TradeInstruction tradeInstruction = TradeInstruction.builder()
                    .entity(Constant.ENTITY_NAME + entityName).type(Type.getRandomType()).agreedFx(random.nextDouble())
                    .currency(randomCurrency).instructionDate(getRandomDateFromNowOn())
                    .settlementDate(settlementDate)
                    .unit(random.nextInt(Constant.UNIT)).pricePerUnit(random.nextDouble())
